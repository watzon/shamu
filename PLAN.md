...
│   │   ├── cursor/           # ACP-stdio via @shamu/protocol-acp (Phase 7.B)
│   │   ├── kimi/             # OpenAI-compatible + own tool loop (deferred to 7.X)
│   │   ├── gemini/           # @google/gemini-cli via @shamu/protocol-acp (Phase 7.C)
...
**Track 8.A — Autonomous loop (Parallel)** — #31
- [x] Daemon mode: long-lived process subscribed to Linear webhooks, picks up `shamu:ready` issues, runs canonical flow
- [x] Rate limiter: global concurrency cap + per-role cap; queue overflow → `shamu:blocked` with reason
- [x] Graceful shutdown: drain running runs into a resumable state before exit
- [ ] 24-hour soak test on staging Linear project — infrastructure shipped (`apps/cli/scripts/soak-daemon.ts`, `docs/phase-8/soak-test.md`); real 24h run owed to the user (staging Linear project + webhook + credentials required)

Follow-on closed in #32: `packages/flows/plan-execute-review::makeSpawnOpts` now threads `SpawnOpts.env` end-to-end so the per-run egress broker actually routes vendor subprocess traffic (was a documented no-op in #31).

**Track 8.B — A2A server (Parallel with 8.A; v1 scope)** — #30
- [x] `packages/protocol/a2a`: A2A v1.0 server + client
- [x] Signed Agent Cards: card signing, card verification on inbound
- [x] JSON-RPC + SSE transport
- [x] Auth: bearer tokens bound to Agent Card issuer
- [x] Example: remote Claude agent hosted on another box joins a local swarm (`packages/protocol/a2a/examples/remote-claude/`)

Supervisor + swarm-mailbox integration of A2A is explicitly a follow-on track — the protocol package is standalone in v1.

**Track 8.C — Ops polish (Serial after 8.A; subsumes the UI plan's Phase 8 items)** — #27 #33 #34 #35 #36 #37
- [x] `shamu doctor`: environment/auth/webhook health check, keychain integrity, clock-skew vs NTP, `audit_events` chain verification, egress-broker reachability, tunnel scope warnings — #34
- [x] `bun build --compile` single-binary release on macOS arm64 + Linux x86_64 — #36 (`scripts/build-release.ts`; real GitHub-Releases pipeline is a separate track)
- [x] **Sidecar bootstrap for the Claude 200MB per-platform binary** — #36 (`ensureClaudeSidecar` with atomic + SHA-pinned download + retry; pin file ships with placeholders regenerated by the release pipeline)
- [x] `shamu ui` opens the web dashboard in the default browser — #27
- [x] Screenshot CI: every PR gets TUI + web screenshots captured via headless render — #35 (web dashboard only; TUI screenshots deferred until the TUI lands)
- [x] README, architecture diagram, contribution guide, threat-model summary — #33
- [x] Container-based network egress enforcement replaces Phase 7's in-process egress broker (same policy format) — #37 (`@shamu/egress-broker-container`; opt-in alongside the in-process broker, full replacement follow-on once `withEgressBroker` grows a `brokerFactory` knob)

**Exit:** 24-hour autonomous run on staging Linear with watchdog + escalation proven (owed manual step); signed single-binary release shipped (scripts ready, Apple Developer ID signing cert required from the user).
