/**
 * Project Cursor ACP events into `AgentEvent`s.
 *
 * ACP session updates arrive as `session/update` notifications whose
 * `update` payload carries a `sessionUpdate` discriminator. The kinds we
 * project today (from the ACP spec + `cursor.com/docs/cli/acp`):
 *
 ...
 *   projector does NOT emit `turn_end`; that's the handle's job because
 *   the result-plumbing is outside the notification stream.
 *
 ...
 *
 * - `turnOpen`: is there an open turn in the correlation state?
 * - `sessionStartEmitted`: has `session_start` been emitted yet?
 * - `toolCallIds`: vendor tool-call id → shamu-minted `ToolCallId`
 * - `toolCallParents`: vendor tool-call id → `EventId` of the
 *   `tool_call` event, for `parentEventId` linkage on `tool_result`
 * - `toolCallsEmitted`: set of vendor tool-call ids we've already emitted
 *   `tool_call` for (the vendor may send `tool_call` followed by
 *   `tool_call_update` updates before the status finalizes)
 * - `toolResultsEmitted`: set of vendor tool-call ids we've already emitted
 *   `tool_result` for.
 */
 ...
 * `AgentEvent`s. Pure function — the caller (`CursorHandle`) enqueues them.
 *
 ...
 */
export function projectCursorEvent(ev: AcpSessionUpdate, ctx: ProjectionContext): AgentEvent[] {
  ...
  switch (kind) {
    ...
    case "tool_call": {
      ...
      // If the vendor landed a pre-resolved tool_call (status already
      // "completed"/"failed"), also emit the tool_result.
    ...
    default: {
      // Unknown kinds surface as a non-fatal error so spec drift is visible.
      out.push({
        ...ctx.corr.envelope(),
        kind: "error",
        fatal: false,
        errorCode: "unknown_update_kind",
        message: `Cursor ACP session/update kind not recognized: ${JSON.stringify(kind)}`,
        retriable: false,
      });
      return out;
    }
...
function extractTextFromContent(content: unknown): string {
  ...
  // Some ACP servers wrap the delta in `{ content: [{ type: "text", text }] }`.
...
function extractToolSummary(upd: AcpSessionUpdate["update"]): string {
  // Prefer an explicit result/content field.
  const content = (upd as { content?: unknown }).content;
